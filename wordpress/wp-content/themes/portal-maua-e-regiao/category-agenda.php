        ...
        <section class="conteudo">
            <section class="principal">
                ...
                <section class="agenda-chamada">
                    <p>Acompanhe a programação dos eventos culturais na cidade de Mauá e Região.</p>
                </section>

                <section class="agenda">
                    <?php
                    $data_atual = strtotime(date("d M Y"));
                    query_posts(query_agenda());

                    if (have_posts()) : while (have_posts()) : the_post();
                        $data_evento = strtotime(get_post_meta($post -> ID, "data", true));
                        // Verifica se a data do post ainda é válida
                        if ($data_evento >= $data_atual): ?>

                        <section class="evento evento-<?php echo get_post_meta($post -> ID, 'cor_box', true); ?>"
                                 title="<?php the_title(); ?>">
